...
public class InstructionService {
    ...
    public InstructionService(InstrukcijaReposiotry instructionRepository, InstruktorRepository instructorRepository, SubjectRepository subjectRepository, KlijentRepository clientRepository, RestTemplate restTemplate, @Qualifier("eurekaClient") EurekaClient eurekaClient, RequestServicePublisher publisher) {
    ...
    public ResponseEntity<?> getStatsInstructionsSubject(String token){
        try{
            ...
            for(int i=0;i<subjects.size();i++){
                ...
                Stat stat = new Stat(subjects.get(i).getId(),subjects.get(i).getName(),subjects.get(i).getDescription(),instructions.size());
                stats.add(stat);
            }
    ...
    public ResponseEntity<?> addInstruction(Long instructorId, Long clientId, Long subjectId, InstructionRequest inst, String token) {
        try {
            if(inst.getScheduledDate().isBefore(OffsetDateTime.now())){
                ErrorMessage error = new ErrorMessage(HttpStatus.BAD_REQUEST,"Invalid scheduled time for instruction", Collections.emptyList());
                return new ResponseEntity<>(error, error.getStatus());
            }
            if(inst.getNumberOfClasses()<1 || inst.getNumberOfClasses()>3){
                ErrorMessage error = new ErrorMessage(HttpStatus.BAD_REQUEST,"Invalid number of classes (max number is 3, min number is 1)", Collections.emptyList());
                return new ResponseEntity<>(error, error.getStatus());
            }
            Client client = getClient(clientId, token);
            Instructor instructor = getInst(instructorId, token);
            Subject subject = getSubject(subjectId, token);
            if (client == null || instructor == null || subject == null) {
                return new ResponseEntity<>(new ErrorMessage(HttpStatus.NOT_FOUND,"Instructor or client with provided id does not exist", Collections.emptyList()), HttpStatus.NOT_FOUND);
            }
